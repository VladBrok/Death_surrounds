                         ...
                         : InfoWindow("", info, showCancelButton)
{
    ...
    errorText.setCharacterSize(utils::percentToPixels(4.f, (int)(window.getSize().x + window.getSize().y)));
    errorText.setColor(sf::Color::Red);
    errorText.setStyle(sf::Text::Bold);
    errorText.setString("ERROR!");
    errorText.setPosition(
        (float)(int)(window.getSize().x / 2u - errorText.getGlobalBounds().width / 2.f),
